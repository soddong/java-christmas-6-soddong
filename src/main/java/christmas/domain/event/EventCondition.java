...
public class EventCondition {
    ...
    public static boolean isOrderPricesAboveThreshold(OrdersDto ordersDto) {
        return ordersDto.orders().stream()
                .mapToInt(order -> Menu.from(order.getName()).getPrice() * order.getQuantity())
                .sum() >= EVENT_COMMON_THRESOLD;
    }

    public static boolean isOrderPricesAboveThreshold(OrdersDto ordersDto, long thresold) {
        return ordersDto.orders().stream()
                .mapToInt(order -> Menu.from(order.getName()).getPrice() * order.getQuantity())
                .sum() >= thresold;
    }
